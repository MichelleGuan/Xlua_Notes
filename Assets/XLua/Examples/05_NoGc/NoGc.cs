...
namespace XLuaTest
{
    //常用的GC管理包括，之前案例里面使用接口定义方法和值，指定类型等，尽量避免装拆箱。调用tick,如案例2写法。完全不用lua时dispose。其他回收作者文档都有写，复杂值类型比较多再参照这个例子。
    //对于简单类型可以直接使用delegate方式或者代理返回接口类型实现无GC，用户自定义Stuct则通过值拷贝的方式不产生GC，写法如下。
    //如果想对struct生成免GC代码，加这个标签（嗯，作者写的），enum也可以
